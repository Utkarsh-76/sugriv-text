...
# Tokenize the dataset
    def tokenize(self,examples):
        ...
        for example in examples:
            ...
            tokenized_text = self.tokenizer(full_text, truncation=True, padding='max_length', max_length=768)
    ...
    def get_data(self,batch_size):
        ...
        train_dataset = self.tokenized_datasets["test"].shuffle(seed=42).select(range(10))
        valid_dataset = self.tokenized_datasets["test"].shuffle(seed=42).select(range(10))

        # Then when creating your DataLoader
        train_dataloader = DataLoader(train_dataset, batch_size=batch_size)
        valid_dataloader = DataLoader(valid_dataset, batch_size=batch_size)
        
        train_dataloader = DataLoader(train_dataloader, batch_size=1)
